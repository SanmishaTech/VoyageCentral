...
const UserForm = ({ mode }: { mode: "create" | "edit" }) => {
  ...
  // Update packages query with proper type
    ...
    if (mode === "edit" && id) {
      const fetchUser = async () => {
        try {
          ...
          // Set only the fields that exist in our form
          Object.keys(userFormSchema.describe().keys).forEach((key) => {
            if (user[key]) {
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {selectedPackage && (
                <div className="col-span-2 p-4 bg-muted rounded-lg">
                  <h4 className="font-medium mb-2">Selected Package Details</h4>
                  <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                    <dt className="text-muted-foreground">
                      Number of Branches:
                    </dt>
                    <dd>{selectedPackage.numberOfBranches}</dd>
                    <dt className="text-muted-foreground">Users per Branch:</dt>
