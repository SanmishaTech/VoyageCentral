...
const fetchSector = async (
  ...
  const response = await get(
    `/accommodations?page=${page}&sortBy=${sortBy}&sortOrder=${sortOrder}&search=${search}&active=${active}${rolesQuery}&limit=${recordsPerPage}`
  );
  return response;
};
...
const AccommodationList = () => {
  ...
  const [sortBy, setSortBy] = useState("accommodationName"); // Default sort column
  ...
  const [selectedAccommodation, setSelectedAccommodation] = useState<
    number | null
  >(null); // Track the selected user for password change
  ...
  const [accommodationToDelete, setAccommodationToDelete] = useState<
    number | null
  >(null); // Track the user ID to delete
  ...
  const totalAccommodation = data?.totalAccommodation || 0;

  // Mutation for deleting
  const deleteMutation = useMutation({
    mutationFn: (id: number) => del(`/accommodations/${id}`),
    onSuccess: () => {
  ...
  const handleDelete = () => {
    if (accommodationToDelete) {
      deleteMutation.mutate(accommodationToDelete);
  ...
  const handleOpenChangePassword = (accommodationId: number) => {
    setSelectedAccommodation(accommodationId); // Set the selected user
          ...
          {isLoading ? (
            ...
            <div className="overflow-x-auto">
              ...
              <CustomPagination
                ...
                totalRecords={totalAccommodation}
            ...
            <div className="text-center">No Accommodation Found.</div>
          )}
        </CardContent>
      </Card>
