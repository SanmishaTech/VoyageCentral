...
const VehicleBookingForm = ({ mode }: { mode: "create" | "edit" }) => {
  ...
  const { data: editVehicleBookingData, isLoading: editVehicleBookingLoading } =
    useQuery({
      ...
      queryFn: async () => {
        const response = await get(`/vehicle-bookings/${vehicleBookingId}`);
        return response; // API returns the sector object directly
      },
    });
  ...
  useEffect(() => {
    if (mode === "create") {
      ...
      if (editBookingData) {
        if (
          editBookingData.bookingDetails &&
          Array.isArray(editBookingData.bookingDetails)
        ) {
          removeVehicleItineraries();

          const mappedItineraries = editBookingData.bookingDetails.map(
            (itinerary, index) => ({
  ...
  const createMutation = useMutation({
    mutationFn: (data: FormInputs) => post(`/vehicle-bookings/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries(["vehicle-bookings"]); // Refetch the users list
  ...
  const updateMutation = useMutation({
    mutationFn: (data: FormInputs) =>
      put(`/vehicle-bookings/${vehicleBookingId}`, data),
    onSuccess: () => {
