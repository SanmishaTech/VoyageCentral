...
const TourMemberList = ({ bookingId }) => {
  ...
  const [tourMemberToDelete, setTourMemberToDelete] = useState<number | null>(
    null
  ); //
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => del(`/tour-members/${id}`),
    onSuccess: () => {
  ...
  const confirmDelete = (id: number) => {
    setTourMemberToDelete(id);
    setShowConfirmation(true);
  };

  const handleDelete = () => {
    if (tourMemberToDelete) {
      deleteMutation.mutate(tourMemberToDelete);
      setShowConfirmation(false);
      setTourMemberToDelete(null);
    }
  };
    ...
    <div className="mt-2 ">
      <div className="mx-auto ">
        <div className="mb-1 w-full flex flex-wrap justify-between items-center gap-2">
          ...
          <Button
            onClick={() => navigate(`/bookings/${bookingId}/tourMember/create`)}
        ...
        <div>
          ...
          {isLoading ? (
                ...
                <TableBody>
                  {tourMembers.map((member) => (
                    <TableRow key={member.id}>
                      ...
                      <TableCell>{member?.foodType}</TableCell>
                      ...
                      <TableCell className="w-20">
                        <div className="flex justify-end gap-2">
                          <Button
                            ...
                            onClick={() =>
                              navigate(
                                `/bookings/${bookingId}/tourMember/${member.id}/edit`
                              )
                            }
                          >
                            <Edit size={16} />
            ...
            <div className="text-center">No tour Member Found.</div>
          )}
        </div>
      </div>
      ...
      <ConfirmDialog
        ...
        description="Are you sure you want to delete this Tour Member? This action cannot be undone."
        onCancel={() => {
          setShowConfirmation(false);
          setTourMemberToDelete(null);
        }}
