...
const GroupClientHotelBookingForm = ({ mode }: { mode: "create" | "edit" }) => {
  const { groupBookingId, groupClientBookingId, hotelBookingId } = useParams<{
    ...
    useQuery({
      ...
      queryFn: async () => {
        const response = await get(
          `/group-client-hotel-bookings/${hotelBookingId}`
        );
        return response; // API returns the sector object directly
      },
    });
  ...
  const createMutation = useMutation({
    mutationFn: (data: FormInputs) =>
      post(`/group-client-hotel-bookings/${groupClientBookingId}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries(["group-client-hotel-bookings"]); // Refetch the users list
  ...
  const updateMutation = useMutation({
    mutationFn: (data: FormInputs) =>
      put(`/group-client-hotel-bookings/${hotelBookingId}`, data),
    onSuccess: () => {
...
export default GroupClientHotelBookingForm;
